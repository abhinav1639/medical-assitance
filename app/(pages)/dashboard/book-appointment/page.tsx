...
return (
    <div className="space-y-6 max-w-7xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Book an appointment with available doctors</p>
      </div>
      ...
      <motion.div
        ...
        className="rounded-2xl gradient-primary p-8 md:p-10 shadow-lg bg-gray-500"
      >
        <h2 className="text-xl md:text-2xl font-bold text-primary-foreground">Not free now Book an appointment?</h2>
        <p className="text-sm text-primary-foreground/80 mt-1.5 max-w-md">Connect with a specialist instantly via voice call. Our doctors are ready to help you.</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {onlineDoctors.map((doc, i) => (
            <BookingCard key={doc.id} doctor={doc} index={i} onConsult={setChatDoctor} />
          ))}
        </div>
      </div>
